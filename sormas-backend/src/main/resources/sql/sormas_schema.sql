...
INSERT INTO featureconfiguration (id, uuid, creationdate, changedate, enabled, featuretype, properties) VALUES (nextval('entity_seq'), generate_base32_uuid(), now(), now(), (SELECT enabled from featureconfiguration where featuretype = 'SORMAS_TO_SORMAS_SHARE_CASES'), 'SORMAS_TO_SORMAS_SHARE_CONTACTS', json_build_object('SHARE_SAMPLES',true,'SHARE_IMMUNIZATIONS',true));
