...
public class AefiInvestigationJurisdictionPredicateValidator extends PredicateJurisdictionValidator {

	private final AefiInvestigationJoins joins;

	public AefiInvestigationJurisdictionPredicateValidator(
		CriteriaBuilder cb,
		AefiInvestigationJoins joins,
		User user,
	...
	public static AefiInvestigationJurisdictionPredicateValidator of(AefiInvestigationQueryContext qc, User user) {
		return new AefiInvestigationJurisdictionPredicateValidator(qc.getCriteriaBuilder(), qc.getJoins(), user, null);
	}
	...
	public Predicate isRootInJurisdictionOrOwned() {
		final Path<Object> reportingUserPath = joins.getRoot().get(AefiInvestigation.REPORTING_USER);
	...
	protected Predicate whenRegionalLevel() {
		return cb.equal(
			joins.getRoot().get(AefiInvestigation.AEFI_REPORT).get(Aefi.IMMUNIZATION).get(Immunization.RESPONSIBLE_REGION).get(Region.ID),
	...
	protected Predicate whenDistrictLevel() {
		return cb.equal(
			joins.getRoot().get(AefiInvestigation.AEFI_REPORT).get(Aefi.IMMUNIZATION).get(Immunization.RESPONSIBLE_DISTRICT).get(District.ID),
	...
	protected Predicate whenCommunityLevel() {
		return cb.equal(
			joins.getRoot().get(AefiInvestigation.AEFI_REPORT).get(Aefi.IMMUNIZATION).get(Immunization.RESPONSIBLE_COMMUNITY).get(Community.ID),
	...
	protected Predicate whenFacilityLevel() {
		return cb.equal(
			joins.getRoot().get(AefiInvestigation.AEFI_REPORT).get(Aefi.IMMUNIZATION).get(Immunization.HEALTH_FACILITY).get(Facility.ID),
			user.getHealthFacility().getId());
	}
