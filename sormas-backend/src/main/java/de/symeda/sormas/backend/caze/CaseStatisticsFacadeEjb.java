...
import de.symeda.sormas.api.statistics.*;
...
public class CaseStatisticsFacadeEjb implements CaseStatisticsFacade {
        ...
        List<StatisticsCaseCountDto> caseCountResults = ((Stream<Object[]>) caseCountQuery.getResultStream())
                .map(result -> {
                    ...
                    return new StatisticsCaseCountDto(result[0] != null ? ((Number) result[0]).intValue() : null, null,
                            StatisticsHelper.buildGroupingKey(rowKey, rowGrouping, rowSubGrouping, regionProvider, districtProvider, communityProvider, healthFacilityProvider),
                            StatisticsHelper.buildGroupingKey(columnKey, columnGrouping, columnSubGrouping, regionProvider, districtProvider, communityProvider, healthFacilityProvider));
                })
    ...
    public Pair<String, List<Object>> buildCaseCountQuery(StatisticsCaseCriteria caseCriteria,
        ...
        if (subGroupingA == StatisticsCaseSubAttribute.HEALTH_FACILITY || subGroupingB == StatisticsCaseSubAttribute.HEALTH_FACILITY) {
            caseJoinBuilder.append(" LEFT JOIN ").append(Facility.TABLE_NAME).append(" ON ").append(Case.TABLE_NAME)
                    .append(".").append(Case.HEALTH_FACILITY).append("_id = ").append(Facility.TABLE_NAME)
                    .append(".").append(Facility.ID);
        } else if (subGroupingA == StatisticsCaseSubAttribute.COMMUNITY || subGroupingB == StatisticsCaseSubAttribute.COMMUNITY) {
            caseJoinBuilder.append(" LEFT JOIN ").append(Community.TABLE_NAME).append(" ON ").append(Case.TABLE_NAME)
                    .append(".").append(Case.COMMUNITY).append("_id = ").append(Community.TABLE_NAME)
                    .append(".").append(Community.ID);
            ...
            caseJoinBuilder.append(" LEFT JOIN ").append(District.TABLE_NAME).append(" ON ").append(Case.TABLE_NAME)
                    .append(".").append(Case.DISTRICT).append("_id = ").append(District.TABLE_NAME)
                    .append(".").append(District.ID);
        } else {
            caseJoinBuilder.append(" LEFT JOIN ").append(Region.TABLE_NAME).append(" ON ").append(Case.TABLE_NAME)
                    .append(".").append(Case.REGION).append("_id = ").append(Region.TABLE_NAME).append(".").append(Region.ID);
        }
        ...
        if (CollectionUtils.isNotEmpty(caseCriteria.getCommunities())) {
            ...
            extendFilterBuilderWithSimpleValue(caseFilterBuilder, filterBuilderParameters, Case.TABLE_NAME, Case.COMMUNITY + "_id",
                    communityIds, entry -> entry);
        }
        ...
        if (CollectionUtils.isNotEmpty(caseCriteria.getHealthFacilities())) {
            ...
            extendFilterBuilderWithSimpleValue(caseFilterBuilder, filterBuilderParameters, Case.TABLE_NAME, Case.HEALTH_FACILITY + "_id",
                    facilityIds, entry -> entry);
        }
            ...
            case JURISDICTION: {
                switch (subGrouping) {
                    ...
                    case DISTRICT:
                        ...
                        break;
                    case COMMUNITY:
                        groupingSelectPartBuilder.append(Community.TABLE_NAME).append(".").append(Community.ID).append(" AS ")
                                .append(groupAlias);
                        break;
                    case HEALTH_FACILITY:
                        groupingSelectPartBuilder.append(Facility.TABLE_NAME).append(".").append(Facility.ID).append(" AS ")
                                .append(groupAlias);
                        break;
                    default:
