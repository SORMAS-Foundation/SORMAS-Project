...
public class CaseListCriteriaBuilder {
	...
	private <T> CriteriaQuery<T> buildIndexCriteria(Class<T> type,
		...
		if (sortProperties != null && sortProperties.size() > 0) {
			...
			for (SortProperty sortProperty : sortProperties) {
				order.addAll(orderExpressionProvider.forProperty(sortProperty, caze, joins).stream()
	...
	public List<Selection<?>> getCaseIndexSelections(Root<Case> root, CaseJoins joins) {
		return Arrays.asList(
				...
				joins.getPerson().get(Person.BIRTHDATE_MM), joins.getPerson().get(Person.BIRTHDATE_YYYY), joins.getPerson().get(Person.SEX),
