...
public class CaseFacadeEjb implements CaseFacade {
	...
	public List<CaseExportDto> getExportList(
		...
		cq.multiselect(caseRoot.get(Case.ID), joins.getPerson().get(Person.ID), joins.getPersonAddress().get(Location.ID),
				...
				caseRoot.get(Case.CASE_PHONE_NUMBER), caseRoot.get(Case.CASE_EMAIL)
				);
		//@formatter:on
		...
		if (!resultList.isEmpty()) {
			...
			// TODO implement export of visit information for case
			// only include when CASE_VISITS feature is enabled
...
//			}
	...
	public Case fillOrBuildEntity(@NotNull CaseDataDto source, Case target) {
		...
		target.setCaseEmail(source.getCaseEmail());
		target.setCasePhoneNumber(source.getCasePhoneNumber());
