	...
	public static final ArchRule testCorrectNamedMethods = classes().that().resideInAPackage("de.symeda.sormas.api.(*)..").
		...
		should(new ArchCondition<JavaClass>("have proper method prefixes") {
			...
			public void check(JavaClass javaClass, ConditionEvents events) {
				...
				String message;
				if (satisfied) {
					message = "All methods in " + javaClass.getName() + " are correctly named";
				} else {
					message = String.format("%s contains the following wrongly named methods: %s", javaClass.getName(), wrongNaming);
				}
