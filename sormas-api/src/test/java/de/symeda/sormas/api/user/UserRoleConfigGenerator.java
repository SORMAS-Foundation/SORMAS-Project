...
public class UserRoleConfigGenerator {
	...
	public void printDefaultUserRoleConfig() {
		...
		if (userRole != null) {
			...
			sqlLineBuilder.append("INSERT INTO userrolesconfig VALUES (nextval('entity_seq'), ").append("'")
					.append(DataHelper.createUuid()).append("', ").append("now(), now(), ").append("'")
					.append(userRole.name()).append("');");
			...
			for (UserRight userRight : userRole.getDefaultUserRights()) {
				...
				sqlLineBuilder.append("INSERT INTO userroles_userrights (userright, userrole_id) VALUES (").append("'")
						.append(userRight.name()).append("', ")
						.append("(SELECT id FROM userrolesconfig WHERE userrole = '").append(userRole.name())
						.append("'));");
