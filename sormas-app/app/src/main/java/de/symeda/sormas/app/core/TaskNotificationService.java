			...
			if (ConfigProvider.getUser() != null && !RetroProvider.isConnectedOrConnecting()) {
				...
				RetroProvider.connectAsync(getApplicationContext(), false, (result, versionCompatible) -> {
					...
					if (result.getResultStatus().isSuccess()) {
						SynchronizeDataAsync.call(SynchronizeDataAsync.SyncMode.Changes, this, (syncFailed, syncFailedMessage) -> {
	...
	private static void doWeeklyReportNotification(Context context, Date notificationRangeStart, Date notificationRangeEnd) {
		if (ConfigProvider.hasUserRight(UserRight.WEEKLYREPORT_CREATE)) {
			...
			if (DateHelper.isBetween(notificationPoint, notificationRangeStart, notificationRangeEnd)) {
				WeeklyReport weeklyReport = DatabaseHelper.getWeeklyReportDao()
					.queryByEpiWeekAndUser(DateHelper.getPreviousEpiWeek(notificationPoint), ConfigProvider.getUser());
				if (weeklyReport == null) {
					...
					NotificationCompat.Builder notificationBuilder =
						new NotificationCompat.Builder(context, NotificationHelper.NOTIFICATION_CHANNEL_TASKS_ID).setTicker(title)
							.setSmallIcon(R.mipmap.ic_launcher_foreground)
