...
public class DatabaseHelper extends OrmLiteSqliteOpenHelper {
	...
	public void onUpgrade(SQLiteDatabase db, ConnectionSource connectionSource, int oldVersion, int newVersion) {
		...
		try {
			...
			case 230:
				currentVersion = 230;
				getDao(Symptoms.class).executeRaw("ALTER TABLE symptoms RENAME TO tmp_symptoms;");
				getDao(Symptoms.class).executeRaw(
						"CREATE TABLE symptoms (abdominalPain VARCHAR, anorexiaAppetiteLoss VARCHAR, bleedingVagina VARCHAR, changeDate BIGINT NOT NULL, "
								+ "chestPain VARCHAR, confusedDisoriented VARCHAR, conjunctivitis VARCHAR, cough VARCHAR, creationDate BIGINT NOT NULL, diarrhea VARCHAR, difficultyBreathing VARCHAR, "
								+ "digestedBloodVomit VARCHAR, eyePainLightSensitive VARCHAR, fever VARCHAR, gumsBleeding VARCHAR, headache VARCHAR, hemoptysis VARCHAR, hiccups VARCHAR, id INTEGER PRIMARY KEY AUTOINCREMENT, "
								+ "injectionSiteBleeding VARCHAR, jointPain VARCHAR, musclePain VARCHAR, otherHemorrhagicSymptoms VARCHAR, otherHemorrhagicSymptomstext VARCHAR, otherNonHemorrhagicSymptoms VARCHAR, "
								+ "otherNonHemorrhagicSymptomsText VARCHAR, skinRash VARCHAR, soreThroat VARCHAR, onSetDate BIGINT, temperature FLOAT, temperatureSource VARCHAR, unexplainedBleeding VARCHAR, uuid VARCHAR NOT NULL, "
								+ "vomiting VARCHAR, dehydration VARCHAR, fatigueWeakness VARCHAR, kopliksSpots VARCHAR, nausea VARCHAR, neckStiffness VARCHAR, onSetSymptom VARCHAR, otitisMedia VARCHAR, refusalFeedOrDrink VARCHAR, "
								+ "runnyNose VARCHAR, seizures VARCHAR, symptomatic SMALLINT, bloodInStool VARCHAR, noseBleeding VARCHAR, bloodyBlackStool VARCHAR, redBloodVomit VARCHAR, coughingBlood VARCHAR, skinBruising VARCHAR, "
								+ "bloodUrine VARCHAR, alteredConsciousness VARCHAR, throbocytopenia VARCHAR, hearingLoss VARCHAR, shock VARCHAR, symptomsComments VARCHAR, sys_period BIGINT NOT NULL, backache VARCHAR, "
								+ "eyesBleeding VARCHAR, jaundice VARCHAR, darkUrine VARCHAR, stomachBleeding VARCHAR, rapidBreathing VARCHAR, swollenGlands VARCHAR, lesions VARCHAR, lesionsSameState VARCHAR, lesionsSameSize VARCHAR, "
								+ "lesionsDeepProfound VARCHAR, lesionsFace SMALLINT, lesionsLegs SMALLINT, lesionsSolesFeet SMALLINT, lesionsPalmsHands SMALLINT, lesionsThorax SMALLINT, lesionsArms SMALLINT, lesionsGenitals SMALLINT, "
								+ "lesionsAllOverBody SMALLINT, lesionsResembleImg1 VARCHAR, lesionsResembleImg2 VARCHAR, lesionsResembleImg3 VARCHAR, lesionsResembleImg4 VARCHAR, lymphadenopathyInguinal VARCHAR, "
								+ "lymphadenopathyAxillary VARCHAR, lymphadenopathyCervical VARCHAR, chillsSweats VARCHAR, lesionsThatItch VARCHAR, bedridden VARCHAR, oralUlcers VARCHAR, patienTillLocation VARCHAR, "
								+ "painfulLymphadenitis VARCHAR, buboesGroinArmpitNeck VARCHAR, blackeningDeathOfTissue VARCHAR, bulgingFontanelle VARCHAR, lesionsOnSetDate BIGINT, meningealSigns VARCHAR, bloodPressureSystolic INTEGER, "
								+ "bloodPressureDiastolic INTEGER, heartRate INTEGER, pharyngealerythema VARCHAR, pharyngealexudate VARCHAR, oedemaFaceNeck VARCHAR, oedemaLowerExtremity VARCHAR, lossSkinTurgor VARCHAR, "
								+ "palpableLiver VARCHAR, palpableSpleen VARCHAR, malaise VARCHAR, sunkenEyesFontanelle VARCHAR, sidePain VARCHAR, fluidInLungCavity VARCHAR, tremor VARCHAR, hemorrhagicSyndrome VARCHAR, "
								+ "hyperglycemia VARCHAR, hypoglycemia VARCHAR, sepsis VARCHAR, midUpperArmCircumference INTEGER, respiratoryRate INTEGER, weight INTEGER, height INTEGER, glasgowComaScale INTEGER, "
								+ "bilateralCataracts VARCHAR, unilateralCataracts VARCHAR, congenitalGlaucoma VARCHAR, pigmentaryRetinopathy VARCHAR, purpuricRash VARCHAR, microcephaly VARCHAR, developmentalDelay VARCHAR, "
								+ "splenomegaly VARCHAR, meningoencephalitis VARCHAR, radiolucentBoneDisease VARCHAR, congenitalHeartDisease VARCHAR, congenitalHeartDiseaseType VARCHAR, congenitalHeartDiseaseDetails VARCHAR, "
								+ "jaundiceWithin24HoursOfBirth VARCHAR, hydrophobia VARCHAR, opisthotonus VARCHAR, anxietyStates VARCHAR, delirium VARCHAR, uproariousness VARCHAR, paresthesiaAroundWound VARCHAR, "
				...
				getDao(Symptoms.class).executeRaw(
						"INSERT INTO symptoms(abdominalPain, anorexiaAppetiteLoss, bleedingVagina, changeDate, "
								+ "chestPain, confusedDisoriented, conjunctivitis, cough, creationDate, diarrhea, difficultyBreathing, "
								+ "digestedBloodVomit, eyePainLightSensitive, fever, gumsBleeding, headache, hemoptysis, hiccups, id, "
								+ "injectionSiteBleeding, jointPain, musclePain, otherHemorrhagicSymptoms, otherHemorrhagicSymptomstext, otherNonHemorrhagicSymptoms, "
								+ "otherNonHemorrhagicSymptomsText, skinRash, soreThroat, onSetDate, temperature, temperatureSource, unexplainedBleeding, uuid, "
								+ "vomiting, dehydration, fatigueWeakness, kopliksSpots, nausea, neckStiffness, onSetSymptom, otitisMedia, refusalFeedOrDrink, "
								+ "runnyNose, seizures, symptomatic, bloodInStool, noseBleeding, bloodyBlackStool, redBloodVomit, coughingBlood, skinBruising, "
								+ "bloodUrine, alteredConsciousness, throbocytopenia, hearingLoss, shock, symptomsComments, sys_period, backache, "
								+ "eyesBleeding, jaundice, darkUrine, stomachBleeding, rapidBreathing, swollenGlands, lesions, lesionsSameState, lesionsSameSize, "
								+ "lesionsDeepProfound, lesionsFace, lesionsLegs, lesionsSolesFeet, lesionsPalmsHands, lesionsThorax, lesionsArms, lesionsGenitals, "
								+ "lesionsAllOverBody, lesionsResembleImg1, lesionsResembleImg2, lesionsResembleImg3, lesionsResembleImg4, lymphadenopathyInguinal, "
								+ "lymphadenopathyAxillary, lymphadenopathyCervical, chillsSweats, lesionsThatItch, bedridden, oralUlcers, patienTillLocation, "
								+ "painfulLymphadenitis, buboesGroinArmpitNeck, blackeningDeathOfTissue, bulgingFontanelle, lesionsOnSetDate, meningealSigns, bloodPressureSystolic, "
								+ "bloodPressureDiastolic, heartRate, pharyngealerythema, pharyngealexudate, oedemaFaceNeck, oedemaLowerExtremity, lossSkinTurgor, "
								+ "palpableLiver, palpableSpleen, malaise, sunkenEyesFontanelle, sidePain, fluidInLungCavity, tremor, hemorrhagicSyndrome, "
								+ "hyperglycemia, hypoglycemia, sepsis, midUpperArmCircumference, respiratoryRate, weight, height, glasgowComaScale, "
								+ "bilateralCataracts, unilateralCataracts, congenitalGlaucoma, pigmentaryRetinopathy, purpuricRash, microcephaly, developmentalDelay, "
								+ "splenomegaly, meningoencephalitis, radiolucentBoneDisease, congenitalHeartDisease, congenitalHeartDiseaseType, congenitalHeartDiseaseDetails, "
								+ "jaundiceWithin24HoursOfBirth, hydrophobia, opisthotonus, anxietyStates, delirium, uproariousness, paresthesiaAroundWound, "
								...
								+ "highOrLowBloodPressure, urinaryRetention "
								+ "SELECT abdominalPain, anorexiaAppetiteLoss, bleedingVagina, changeDate, "
								+ "chestPain, confusedDisoriented, conjunctivitis, cough, creationDate, diarrhea, difficultyBreathing, "
								+ "digestedBloodVomit, eyePainLightSensitive, fever, gumsBleeding, headache, hemoptysis, hiccups, id, "
								+ "injectionSiteBleeding, jointPain, musclePain, otherHemorrhagicSymptoms, otherHemorrhagicSymptomstext, otherNonHemorrhagicSymptoms, "
								+ "otherNonHemorrhagicSymptomsText, skinRash, soreThroat, onSetDate, temperature, temperatureSource, unexplainedBleeding, uuid, "
				...
				getDao(Symptoms.class).executeRaw("DROP TABLE tmp_symptoms;");
