...
public class MaternalHistoryDtoHelper extends AdoDtoHelper<MaternalHistory, MaternalHistoryDto> {
    ...
    @Override
    protected Call<List<MaternalHistoryDto>> pullAllSince(long since) {
        throw new UnsupportedOperationException("Entity is embedded");
    }

    @Override
    protected Call<List<MaternalHistoryDto>> pullByUuids(List<String> uuids) {
        throw new UnsupportedOperationException("Entity is embedded");
    }

    @Override
    protected Call<List<PushResult>> pushAll(List<MaternalHistoryDto> dtos) {
        throw new UnsupportedOperationException("Entity is embedded");
    }
    ...
    public void fillInnerFromAdo(MaternalHistoryDto target, MaternalHistory source) {
        ...
        if (source.getRashExposureRegion() != null) {
            target.setRashExposureRegion(RegionDtoHelper.toReferenceDto(DatabaseHelper.getRegionDao().queryForId(source.getRashExposureRegion().getId())));
        } else {
            target.setRashExposureRegion(null);
        }
        if (source.getRashExposureDistrict() != null) {
            target.setRashExposureDistrict(DistrictDtoHelper.toReferenceDto(DatabaseHelper.getDistrictDao().queryForId(source.getRashExposureDistrict().getId())));
        } else {
            target.setRashExposureDistrict(null);
        }
        if (source.getRashExposureCommunity() != null) {
            target.setRashExposureCommunity(CommunityDtoHelper.toReferenceDto(DatabaseHelper.getCommunityDao().queryForId(source.getRashExposureCommunity().getId())));
        } else {
