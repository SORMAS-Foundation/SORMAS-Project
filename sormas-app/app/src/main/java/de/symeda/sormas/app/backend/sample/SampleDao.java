...
public class SampleDao extends AbstractAdoDao<Sample> {
	...
	public List<Sample> queryByCriteria(SampleCriteria criteria, long offset, long limit) {
		try {
			return buildQueryBuilder(criteria).orderBy(Sample.SAMPLE_DATE_TIME, true).offset(offset).limit(limit).query();
		} catch (SQLException e) {
	...
	private QueryBuilder<Sample, Long> buildQueryBuilder(SampleCriteria criteria) throws SQLException {
		...
		if (criteria.getCaze() != null) {
			...
			if (criteria.getShipmentStatus() != null) {
				...
				case NOT_SHIPPED:
					where.and()
						.and(
							...
							where.or(where.eq(Sample.REFERRED_TO_UUID, ""), where.isNull(Sample.REFERRED_TO_UUID)));
					break;
				case SHIPPED:
					where.and()
						.and(
				...
				case RECEIVED:
					where.and()
						.and(where.eq(Sample.RECEIVED, true), where.or(where.eq(Sample.REFERRED_TO_UUID, ""), where.isNull(Sample.REFERRED_TO_UUID)));
					break;
				case REFERRED_OTHER_LAB:
					where.and().and(where.isNotNull(Sample.REFERRED_TO_UUID), where.ne(Sample.REFERRED_TO_UUID, ""));
					break;
				default:
