	...
	public List<PersonNameDto> getRelevantPersonNames(PersonSimilarityCriteria similarityCriteria) {
		try {
			...
			if (similarityCriteria.getSex() != null) {
				where.and();
				where.or(where.isNull(Person.SEX), where.eq(Person.SEX, similarityCriteria.getSex()));
			}
			if (similarityCriteria.getBirthdateYYYY() != null) {
				where.and();
				where.or(where.isNull(Person.BIRTHDATE_YYYY), where.eq(Person.BIRTHDATE_YYYY, similarityCriteria.getBirthdateYYYY()));
			}
			if (similarityCriteria.getBirthdateMM() != null) {
				where.and();
				where.or(where.isNull(Person.BIRTHDATE_MM), where.eq(Person.BIRTHDATE_MM, similarityCriteria.getBirthdateMM()));
			}
			if (similarityCriteria.getBirthdateDD() != null) {
				where.and();
				where.or(where.isNull(Person.BIRTHDATE_DD), where.eq(Person.BIRTHDATE_DD, similarityCriteria.getBirthdateDD()));
			}
