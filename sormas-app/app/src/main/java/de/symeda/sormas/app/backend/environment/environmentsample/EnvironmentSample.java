...
public class EnvironmentSample extends PseudonymizableAdo {
    ...
    public Set<Pathogen> getRequestedPathogenTests() {
        if (requestedPathogenTests == null) {
            ...
            requestedPathogenTests = list != null ? list.stream().map(p -> customizableEnumValueDao.<Pathogen>getEnumValue(CustomizableEnumType.PATHOGEN, p)).collect(Collectors.toSet()) : new HashSet<>();
        }
