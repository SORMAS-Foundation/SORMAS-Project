...
public class ContinentResource extends EntityDtoResource<ContinentDto> {
	...
	public List<String> archive(@RequestBody List<String> uuids) {
		return FacadeProvider.getContinentFacade().archive(uuids).stream().map(ProcessedEntity::getEntityUuid).collect(Collectors.toList());
	}

	@POST
	@Path("/dearchive")
	public List<String> dearchive(@RequestBody List<String> uuids) {
		return FacadeProvider.getContinentFacade().dearchive(uuids).stream().map(ProcessedEntity::getEntityUuid).collect(Collectors.toList());
	}
