...
public class RegionResource extends EntityDtoResource<RegionDto> {
	...
	public List<RegionDto> getByUuids(List<String> uuids) {
		return FacadeProvider.getRegionFacade().getByUuids(uuids);
	}
	...
	public List<String> archive(@RequestBody List<String> uuids) {
		return FacadeProvider.getRegionFacade().archive(uuids).stream().map(ProcessedEntity::getEntityUuid).collect(Collectors.toList());
	}

	@POST
	@Path("/dearchive")
	public List<String> dearchive(@RequestBody List<String> uuids) {
		return FacadeProvider.getRegionFacade().dearchive(uuids).stream().map(ProcessedEntity::getEntityUuid).collect(Collectors.toList());
	}

	@Override
	public UnaryOperator<RegionDto> getSave() {
		return FacadeProvider.getRegionFacade()::save;
	}

	@Override
	public Response postEntityDtos(List<RegionDto> regionDtos) {
		return super.postEntityDtos(regionDtos);
	}
}
