...
public class WeeklyReportsSteps implements En {
	...
	public WeeklyReportsSteps(WebDriverHelpers webDriverHelpers) {
		...
		When("I choose {string} as year for weekly reports", (String year) -> {
			webDriverHelpers.waitForPageLoaded();
			webDriverHelpers.selectFromCombobox(WeeklyReportsPage.YEAR_FILTER, year);
		});

		When("I choose {string} as epi week for weekly reports", (String epiWeek) -> {
			webDriverHelpers.waitForPageLoaded();
		...
		Then("I check that all filter components for weekly reports are shown", () -> {
			...
			webDriverHelpers.waitUntilIdentifiedElementIsPresent(WeeklyReportsPage.LAST_EPI_WEEK_BUTTON);
			webDriverHelpers.waitUntilAListOfElementsIsPresent(WeeklyReportsPage.LAST_EPI_WEEK_BUTTON, 1);
			webDriverHelpers.waitUntilElementIsVisibleAndClickable(WeeklyReportsPage.LAST_EPI_WEEK_BUTTON);
		});
		...
		Then("^I check that header names of grid for weekly reports are shown$", () -> {
			List<String> expectedHeaderNames = Arrays.asList(
				"Officers",
				"Informants",
				"Region",
				"Number of officers",
	...
	private void checkThatHeaderNamesExistInGrid(List<String> expectedHeaderNames) {
		for (String expectedHeaderName : expectedHeaderNames) {
			webDriverHelpers.waitUntilElementIsVisibleAndClickable(By.xpath(String.format(WeeklyReportsPage.GRID_HEADER, expectedHeaderName)));
		}
	}
