...
public class AboutDirectorySteps implements En {
	...
	public static final String DATA_PROTECTION_DICTIONARY_FILE_PATH = String.format("sormas_data_protection_dictionary_%s_.xlsx", LocalDate.now());
	public static final String DATA_DICTIONARY_FILE_PATH = String.format("sormas_data_dictionary_%s_.xlsx", LocalDate.now());
	public static final String DEUTSCH_DATA_DICTIONARY_FILE_PATH = String.format("sormas_datenbeschreibungsverzeichnis_%s_.xlsx", LocalDate.now());
	...
	private static final String RELEASE_PAGE = "https://github.com/sormas-foundation/SORMAS-Project/releases";
	...
	public AboutDirectorySteps(WebDriverHelpers webDriverHelpers, SoftAssert softly, AssertHelpers assertHelpers) {
		...
		When("I select {string} language from Combobox in User settings", (String chosenLanguage) -> {
			webDriverHelpers.selectFromCombobox(LANGUAGE_COMBOBOX, chosenLanguage);
			webDriverHelpers.clickOnWebElementBySelector(SAVE_BUTTON);
			webDriverHelpers.waitForPageLoadingSpinnerToDisappear(20);
		});

		When("^I click on ([^\"]*) hyperlink and download XLSX file from About directory$", (String dictionaryName) -> {
		...
		When("^I validate data from downloaded XLSX ([^\"]*) file$", (String dictionaryName) -> {
			switch (dictionaryName) {
			case "Data Protection Dictionary":
		...
		When("^I check if Data Dictionary contains entries name in English$", () -> checkNameOfTheTabsMatchFieldIdEntries(DATA_DICTIONARY_FILE_PATH));
		When(
		...
		Then("^I delete ([^\"]*) downloaded file from About Directory$", (String dictionaryName) -> {
			switch (dictionaryName) {
			case "Data Protection Dictionary":
		...
		When("^I check if last downloaded XLSX from About Directory content is translated into ([^\"]*)$", (String language) -> {
			String[] receivedWordsFromArray = {
				xlsxFileContentList.get(3),
				xlsxFileContentList.get(17) };
			for (String word : receivedWordsFromArray) {
				LanguageDetectorHelper.checkLanguage(word, language);
			}
		});

		When("^I click on Sormas version in About directory and i get redirected to github$", () -> {
			webDriverHelpers.waitUntilElementIsVisibleAndClickable(SORMAS_VERSION_LINK);
			String hrefValue = "";
			try {
			...
			Assert.assertTrue(hrefValue.contains("github.com/sormas-foundation/SORMAS-Project"), "Sormas version hyperlink value is wrong");
			webDriverHelpers.clickOnWebElementBySelector(SORMAS_VERSION_LINK);
			TimeUnit.SECONDS.sleep(1);
			String link = webDriverHelpers.getAttributeFromWebElement(SORMAS_VERSION_HYPERLINK_TARGET, "href");
			webDriverHelpers.switchToOtherWindow();
			softly.assertEquals(link, webDriverHelpers.returnURL(), "Sormas version link is not the correct");
			softly.assertAll();
			webDriverHelpers.closeActiveWindow();
		});

		When("^I click on What's new in About directory and i get redirected to Sormas what's new page$", () -> {
			...
			TimeUnit.SECONDS.sleep(1);
			webDriverHelpers.switchToOtherWindow();
			softly.assertTrue(webDriverHelpers.returnURL().contains(RELEASE_PAGE), "What's new link is not the correct");
			softly.assertAll();
			webDriverHelpers.closeActiveWindow();
		});
		When("^I click on Official SORMAS Website in About directory and i get redirected to the offical Sormas website$", () -> {
			...
			TimeUnit.SECONDS.sleep(1);
			webDriverHelpers.switchToOtherWindow();
			softly.assertEquals("https://sormas.org/", webDriverHelpers.returnURL(), "Official sormas website link is not correct");
			softly.assertAll();
			webDriverHelpers.closeActiveWindow();
		});
		When("^I click on SORMAS Github in About directory and i get redirected to github page of sormas$", () -> {
		...
		When("^I click on Full Changelog in About directory and i get redirected to github project release page of sormas$", () -> {
			...
			TimeUnit.SECONDS.sleep(1);
			webDriverHelpers.switchToOtherWindow();
			softly.assertEquals(RELEASE_PAGE, webDriverHelpers.returnURL(), "Sormas full changelog link is not correct");
			softly.assertAll();
			webDriverHelpers.closeActiveWindow();
		});
		When("^I click on Case Classification Rules hyperlink and download HTML file in About directory$", () -> {
			webDriverHelpers.waitUntilIdentifiedElementIsVisibleAndClickable(CASE_CLASSIFICATION_RULES_HYPERLINK, 15);
		...
		Then("^I check that Surveillance Dashboard header is correctly displayed in ([^\"]*) language$", (String language) -> {
			...
			webDriverHelpers.waitUntilIdentifiedElementIsVisibleAndClickable(SURVEILLANCE_DASHBOARD_NAME, 30);
			LanguageDetectorHelper.checkLanguage(webDriverHelpers.getTextFromWebElement(SURVEILLANCE_DASHBOARD_NAME), language);
		});
	}
			...
			for (int i = 0; i < 5; i++) {
				...
				assertHelpers.assertWithPoll20Second(
					() -> Assert.assertEquals(
						...
						currentCell.getStringCellValue().substring(0, datatypeSheet.getSheetName().replaceAll("\\s+", "").length()).toUpperCase(),
