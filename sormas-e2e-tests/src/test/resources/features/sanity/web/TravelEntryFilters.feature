...
#    ToDO(DELETE!!!):
#    Ponizsze Stepy sa najprawdopodobniej do usuniecia !!!-> był to swoisty workaround, który po naprawieniu buga jest bez sensu
#    And I click on the Entries button from navbar
#    And I filter by Person ID on Travel Entry directory page
...
#    And I click on Save button from the edit travel entry form
    ...
    And I filter by Person ID on Travel Entry directory page
    And I click on SHOW MORE FILTERS BUTTON Travel Entry directory page
    And I fill Travel Entry from input to 5 days before UI Travel Entry created on Travel Entry directory page
    And I fill Travel Entry to input to 3 days after UI Travel Entry created on Travel Entry directory page
    And I click APPLY BUTTON in Travel Entry Directory Page
    And I check that number of displayed Travel Entry results is 1
    And I fill Travel Entry to input to 2 days before UI Travel Entry created on Travel Entry directory page
    And I click APPLY BUTTON in Travel Entry Directory Page
    And I check that number of displayed Travel Entry results is 0
    And I apply "Nach Epi Woche" to data filter option combobox on Travel Entry directory page

#    ToDO -> przykladowo weekend roku pozyskiwany jest w "I change a Report Date for previous week date"
#  uzupelnic weekd roku i po nim wyszukac ostatni weekend w comboboxie
#  I zrobic filtracje danych i bedzie git!!!