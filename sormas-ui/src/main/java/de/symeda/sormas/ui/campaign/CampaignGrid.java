...
public class CampaignGrid extends FilteredGrid<CampaignIndexDto, CampaignCriteria> {
	...
	public CampaignGrid(CampaignCriteria criteria) {
		...
		setColumns(CampaignIndexDto.UUID, CampaignIndexDto.NAME, CampaignIndexDto.START_DATE,
		...
		for(Column<?, ?> column : getColumns()) {
			column.setCaption(I18nProperties.getPrefixCaption(
					CampaignIndexDto.I18N_PREFIX, column.getId().toString(), column.getCaption()));
		}
		
		addItemClickListener(e ->  {
			if ((e.getColumn() != null && CampaignIndexDto.UUID.equals(e.getColumn()
					.getId()))
	...
	public void setLazyDataProvider() {
		DataProvider<CampaignIndexDto, CampaignCriteria> dataProvider = DataProvider
				.fromFilteringCallbacks(query -> FacadeProvider
						.getCampaignFacade()
	...
	public void setEagerDataProvider() {
		ListDataProvider<CampaignIndexDto> dataProvider = DataProvider
				.fromStream(FacadeProvider.getCampaignFacade().getIndexList(getCriteria(), null, null, null).stream());
		setDataProvider(dataProvider);
