...
public class SamplesView extends AbstractView {
	...
	public SamplesView() {
		...
		if (LoginHelper.hasUserRight(UserRight.SAMPLE_EXPORT)) {
			...
			StreamResource streamResource = DownloadUtil.createGridExportStreamResource(sampleListComponent.getGrid(), "sormas_samples", "sormas_samples_" + DateHelper.formatDateForExport(new Date()) + ".csv", "text/csv");
