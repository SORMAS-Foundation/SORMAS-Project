...
public class CampaignController {
    ...
    public void createOrEditCampaign(SormasUI ui, String uuid) {
        ...
        if (uuid != null) {
            ...
            if (ui.getUserProvider().hasUserRight(UserRight.CAMPAIGN_ARCHIVE)) {
            ...
            campaignComponent = getCampaignComponent(ui, null, () -> {
    ...
    public void createCampaignDataForm(SormasUI ui, CampaignReferenceDto campaign, CampaignFormMetaReferenceDto campaignForm) {
        Window window = VaadinUiUtil.createPopupWindow();

        CommitDiscardWrapperComponent<CampaignFormDataEditForm> component =
                getCampaignFormDataComponent(ui, null, campaign, campaignForm, false, false, () -> {
                    window.close();
                    SormasUI.refreshView();
                    Notification
    ...
    public CommitDiscardWrapperComponent<CampaignEditForm> getCampaignComponent(
            @NotNull SormasUI ui, CampaignDto campaignDto, Runnable callback) {

        CampaignEditForm campaignEditForm = new CampaignEditForm(campaignDto);
        if (campaignDto == null) {
            campaignDto = CampaignDto.build();
            campaignDto.setCreatingUser(ui.getUserProvider().getUserReference());
        }
        campaignEditForm.setValue(campaignDto);

        final CommitDiscardWrapperComponent<CampaignEditForm> campaignComponent =
                new CommitDiscardWrapperComponent<CampaignEditForm>(campaignEditForm, ui.getUserProvider().hasUserRight(UserRight.CAMPAIGN_EDIT), campaignEditForm.getFieldGroup()) {
    ...
    public CommitDiscardWrapperComponent<CampaignFormDataEditForm> getCampaignFormDataComponent(
        ...
        if (campaignFormData == null) {
            final UserDto currentUser = ui.getUserProvider().getUser();
            campaignFormData =
                    CampaignFormDataDto.build(campaign, campaignForm, currentUser.getRegion(), currentUser.getDistrict(), currentUser.getCommunity());
            campaignFormData.setCreatingUser(ui.getUserProvider().getUserReference());
        }
        ...
        if (showDeleteButton && ui.getUserProvider().hasUserRight(UserRight.CAMPAIGN_DELETE)) {
    ...
    public void navigateToCampaign(Navigator navigator, String uuid) {
        String navigationState = CampaignView.VIEW_NAME + "/" + uuid;
        navigator.navigateTo(navigationState);
    }

    public void navigateToFormDataView(Navigator navigator, String uuid) {
        String navigationState = CampaignFormDataView.VIEW_NAME + "/" + uuid;
        navigator.navigateTo(navigationState);
    }

    public void navigateToCampaignData(Navigator navigator, String campaignUuid) {
