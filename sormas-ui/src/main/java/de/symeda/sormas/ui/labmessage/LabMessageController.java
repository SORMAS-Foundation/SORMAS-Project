...
public class LabMessageController {
	...
	private CommitDiscardWrapperComponent<SampleCreateForm> getSampleCreateComponent(
		...
		if (!testReportDtos.isEmpty()) {
			...
			((CheckBox) sampleCreateComponent.getWrappedComponent().getField(PathogenTestDto.PRELIMINARY)).setValue(testReportDto.getPreliminary());
		}
	...
	private PathogenTestDto buildPathogenTest(SampleDto sampleDto, LabMessageDto labMessageDto) {
		...
		if (!testReportDtos.isEmpty()) {
			...
			pathogenTestDto.setPreliminary(testReportDto.getPreliminary());
		}
