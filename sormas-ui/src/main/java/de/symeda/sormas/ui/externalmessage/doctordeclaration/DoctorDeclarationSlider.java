...
public class DoctorDeclarationSlider {

	private final List<ExternalMessageDto> doctorDeclarations;
	private final ExternalMessageForm form;
	...
	public DoctorDeclarationSlider(List<ExternalMessageDto> doctorDeclarations) {
		VerticalLayout layout = new VerticalLayout();
		layout.setMargin(true);
		VaadinUiUtil.showPopupWindow(layout, I18nProperties.getString(Strings.headingShowExternalMessage));

		this.index = 0;
		this.doctorDeclarations = doctorDeclarations;
		...
		form.setValue(doctorDeclarations.get(index));
		this.form = form;
		layout.addComponent(this.form);
		layout.addStyleName("doctor-declaration-slider");
	...
	private void next() {
		this.index++;
		this.form.setValue(this.doctorDeclarations.get(this.index));
		this.enableButtons();
	}
	...
	private void enableButtons() {
		this.topRightButton.setEnabled(this.index < this.doctorDeclarations.size() - 1);
		this.topLeftButton.setEnabled(this.index > 0);

		this.bottomRightButton.setEnabled(this.index < this.doctorDeclarations.size() - 1);
