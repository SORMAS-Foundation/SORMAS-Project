...
public class InfrastructureController {
    ...
    public void createFacility(@NotNull SormasUI ui) {
        CommitDiscardWrapperComponent<FacilityEditForm> createComponent = getFacilityEditComponent(ui, null);
        VaadinUiUtil.showModalPopupWindow(createComponent, I18nProperties.getString(Strings.headingCreateNewFacility));
    }

    public void editFacility(@NotNull SormasUI ui, String uuid) {
        FacilityDto facility = FacadeProvider.getFacilityFacade().getByUuid(uuid);
        CommitDiscardWrapperComponent<FacilityEditForm> editComponent = getFacilityEditComponent(ui, facility);
    ...
    public void createArea(@NotNull SormasUI ui) {
        CommitDiscardWrapperComponent<AreaEditForm> createComponent = getAreaEditComponent(ui, null);
        VaadinUiUtil.showModalPopupWindow(createComponent, I18nProperties.getString(Strings.headingCreateEntry));
    }

    public void editArea(@NotNull SormasUI ui, String uuid) {
        AreaDto area = FacadeProvider.getAreaFacade().getAreaByUuid(uuid);
        CommitDiscardWrapperComponent<AreaEditForm> editComponent = getAreaEditComponent(ui, area);
        String caption = I18nProperties.getString(Strings.edit) + " " + area.getName();
        VaadinUiUtil.showModalPopupWindow(editComponent, caption);
    }

    public void createCountry(@NotNull SormasUI ui) {
        CommitDiscardWrapperComponent<CountryEditForm> createComponent = getCountryEditComponent(ui, null);
        VaadinUiUtil.showModalPopupWindow(createComponent, I18nProperties.getString(Strings.headingCreateEntry));
    }

    public void editCountry(@NotNull SormasUI ui, String uuid) {
        CountryDto country = FacadeProvider.getCountryFacade().getCountryByUuid(uuid);
        CommitDiscardWrapperComponent<CountryEditForm> editComponent = getCountryEditComponent(ui, country);
        String caption = I18nProperties.getString(Strings.headingEditCountry);
        VaadinUiUtil.showModalPopupWindow(editComponent, caption);
    }

    public void createRegion(@NotNull SormasUI ui) {
        CommitDiscardWrapperComponent<RegionEditForm> createComponent = getRegionEditComponent(ui, null);
        VaadinUiUtil.showModalPopupWindow(createComponent, I18nProperties.getString(Strings.headingCreateEntry));
    }

    public void editRegion(@NotNull SormasUI ui, String uuid) {
        RegionDto region = FacadeProvider.getRegionFacade().getRegionByUuid(uuid);
        CommitDiscardWrapperComponent<RegionEditForm> editComponent = getRegionEditComponent(ui, region);
        String caption = I18nProperties.getString(Strings.edit) + " " + region.getName();
        VaadinUiUtil.showModalPopupWindow(editComponent, caption);
    }

    public void createDistrict(@NotNull SormasUI ui) {
        CommitDiscardWrapperComponent<DistrictEditForm> createComponent = getDistrictEditComponent(ui, null);
        VaadinUiUtil.showModalPopupWindow(createComponent, I18nProperties.getString(Strings.headingCreateEntry));
    }

    public void editDistrict(@NotNull SormasUI ui, String uuid) {
        DistrictDto district = FacadeProvider.getDistrictFacade().getDistrictByUuid(uuid);
        CommitDiscardWrapperComponent<DistrictEditForm> editComponent = getDistrictEditComponent(ui, district);
        String caption = I18nProperties.getString(Strings.edit) + " " + district.getName();
        VaadinUiUtil.showModalPopupWindow(editComponent, caption);
    }

    public void createCommunity(@NotNull SormasUI ui) {
        CommitDiscardWrapperComponent<CommunityEditForm> createComponent = getCommunityEditComponent(ui, null);
        VaadinUiUtil.showModalPopupWindow(createComponent, I18nProperties.getString(Strings.headingCreateEntry));
    }

    public void editCommunity(@NotNull SormasUI ui, String uuid) {
        CommunityDto community = FacadeProvider.getCommunityFacade().getByUuid(uuid);
        CommitDiscardWrapperComponent<CommunityEditForm> editComponent = getCommunityEditComponent(ui, community);
        String caption = I18nProperties.getString(Strings.edit) + " " + community.getName();
        VaadinUiUtil.showModalPopupWindow(editComponent, caption);
    }

    public void createPointOfEntry(@NotNull SormasUI ui) {
        CommitDiscardWrapperComponent<PointOfEntryForm> component = getPointOfEntryEditComponent(ui, null);
        VaadinUiUtil.showModalPopupWindow(component, I18nProperties.getString(Strings.headingCreateEntry));
    }

    public void editPointOfEntry(@NotNull SormasUI ui, String uuid) {

        PointOfEntryDto pointOfEntry = FacadeProvider.getPointOfEntryFacade().getByUuid(uuid);
        CommitDiscardWrapperComponent<PointOfEntryForm> component = getPointOfEntryEditComponent(ui, pointOfEntry);
    ...
    private CommitDiscardWrapperComponent<FacilityEditForm> getFacilityEditComponent(
            SormasUI ui, FacilityDto facility) {
        ...
        final CommitDiscardWrapperComponent<FacilityEditForm> editView = new CommitDiscardWrapperComponent<FacilityEditForm>(
                editForm,
                ui.getUserProvider().hasUserRight(isNew ? UserRight.INFRASTRUCTURE_CREATE : UserRight.INFRASTRUCTURE_EDIT),
    ...
    private CommitDiscardWrapperComponent<AreaEditForm> getAreaEditComponent(@NotNull SormasUI ui, AreaDto area) {
        ...
        editComponent.addCommitListener(() -> {
            ...
            Notification.show(I18nProperties.getString(Strings.messageEntryCreated), Type.ASSISTIVE_NOTIFICATION);
            ui.getNavigator().navigateTo(AreasView.VIEW_NAME);
        });

        if (!isNew) {
            extendEditComponentWithArchiveButton(ui, editComponent, area.isArchived(), area.getUuid(), InfrastructureType.AREA);
        }

        return editComponent;
    }

    private CommitDiscardWrapperComponent<CountryEditForm> getCountryEditComponent(@NotNull SormasUI ui, CountryDto country) {
        ...
        if (!isNew) {
            extendEditComponentWithArchiveButton(ui, editView, country.isArchived(), country.getUuid(), InfrastructureType.COUNTRY);
        }

        return editView;
    }

    private CommitDiscardWrapperComponent<RegionEditForm> getRegionEditComponent(@NotNull SormasUI ui, RegionDto region) {
        ...
        if (!isNew) {
            extendEditComponentWithArchiveButton(ui, editView, region.isArchived(), region.getUuid(), InfrastructureType.REGION);
        }

        return editView;
    }

    private CommitDiscardWrapperComponent<DistrictEditForm> getDistrictEditComponent(@NotNull SormasUI ui, DistrictDto district) {
        ...
        if (!isNew) {
            extendEditComponentWithArchiveButton(ui, editView, district.isArchived(), district.getUuid(), InfrastructureType.DISTRICT);
        }

        return editView;
    }

    private CommitDiscardWrapperComponent<CommunityEditForm> getCommunityEditComponent(@NotNull SormasUI ui, CommunityDto community) {
        ...
        editView.addCommitListener(new CommitListener() {
            ...
            public void onCommit() {
                ...
                Notification.show(I18nProperties.getString(Strings.messageEntryCreated), Type.ASSISTIVE_NOTIFICATION);
                ui.getNavigator().navigateTo(CommunitiesView.VIEW_NAME);
            }
        });

        if (!isNew) {
            extendEditComponentWithArchiveButton(ui, editView, community.isArchived(), community.getUuid(), InfrastructureType.COMMUNITY);
        }

        return editView;
    }

    private CommitDiscardWrapperComponent<PointOfEntryForm> getPointOfEntryEditComponent(@NotNull SormasUI ui, PointOfEntryDto pointOfEntry) {
        ...
        view.addCommitListener(() -> {
            ...
            Notification.show(I18nProperties.getString(Strings.messageEntryCreated), Type.ASSISTIVE_NOTIFICATION);
            ui.getNavigator().navigateTo(PointsOfEntryView.VIEW_NAME);
        });

        if (!isNew) {
            extendEditComponentWithArchiveButton(ui, view, pointOfEntry.isArchived(), pointOfEntry.getUuid(), InfrastructureType.POINT_OF_ENTRY);
        }
    ...
    private void extendEditComponentWithArchiveButton(
        ...
        if (ui.getUserProvider().hasUserRight(UserRight.INFRASTRUCTURE_ARCHIVE)) {
