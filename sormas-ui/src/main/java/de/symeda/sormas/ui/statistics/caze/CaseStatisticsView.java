...
public class CaseStatisticsView extends StatisticsView {
	...
	protected void populateStatisticsAttributes () {
		...
		attributes.add(createAttribute(StatisticsCaseAttribute.ONSET_TIME));
		attributes.add(createAttribute(StatisticsCaseAttribute.REPORT_TIME));
		StatisticsAttributeGroup group = new StatisticsAttributeGroup(StatisticsCaseAttributeGroup.TIME, attributes);
		groups.add(group);
		
		//place
		attributes = new ArrayList<StatisticsAttribute>();
		attributes.add(createAttribute(StatisticsCaseAttribute.REGION_DISTRICT));
		group = new StatisticsAttributeGroup(StatisticsCaseAttributeGroup.PLACE, attributes);
		groups.add(group);
		
		//person
		attributes = new ArrayList<StatisticsAttribute>();
		attributes.add(createAttribute(StatisticsCaseAttribute.SEX));
		attributes.add(createAttribute(StatisticsCaseAttribute.AGE_INTERVAL_1_YEAR));
		attributes.add(createAttribute(StatisticsCaseAttribute.AGE_INTERVAL_5_YEARS));
		attributes.add(createAttribute(StatisticsCaseAttribute.AGE_INTERVAL_CHILDREN_COARSE));
		attributes.add(createAttribute(StatisticsCaseAttribute.AGE_INTERVAL_CHILDREN_FINE));
		attributes.add(createAttribute(StatisticsCaseAttribute.AGE_INTERVAL_CHILDREN_MEDIUM));
		attributes.add(createAttribute(StatisticsCaseAttribute.AGE_INTERVAL_BASIC));
		group = new StatisticsAttributeGroup(StatisticsCaseAttributeGroup.PERSON, attributes);
		groups.add(group);
		
		//case
		attributes = new ArrayList<StatisticsAttribute>();
		attributes.add(createAttribute(StatisticsCaseAttribute.DISEASE));
		attributes.add(createAttribute(StatisticsCaseAttribute.CLASSIFICATION, CaseClassification.values()));
		attributes.add(createAttribute(StatisticsCaseAttribute.OUTCOME, CaseOutcome.values()));
		attributes.add(createAttribute(StatisticsCaseAttribute.REPORTING_USER_ROLE));
		group = new StatisticsAttributeGroup(StatisticsCaseAttributeGroup.CASE, attributes);
		groups.add(group);
		
		statisticsAttributes = new StatisticsAttributesContainer(groups);
	}
	
	StatisticsAttribute createAttribute (StatisticsCaseAttribute attribute, StatisticsGroupingKey[] groupingKeys) {
	...
	StatisticsAttribute createAttribute (StatisticsCaseAttribute attribute) {
		List<StatisticsSubAttribute> subs = attribute.getSubAttributes() == null ? new ArrayList<>() :
			Arrays.stream(attribute.getSubAttributes())
				.map(n -> new StatisticsSubAttribute(n, n, n.isUsedForFilters(), n.isUsedForGrouping(), new StatisticsHelper.getSubAttributeValues()))
				.collect(Collectors.toList());
	...
	protected List<StatisticsCountDto> generateStatistics() {
		...
		List<StatisticsCountDto> resultData = FacadeProvider.getCaseStatisticsFacade().queryCaseCount(
			criteria,
	...
	protected void fillCaseCriteria(boolean showCaseIncidence) {
		...
		for (StatisticsFilterComponent filterComponent : filterComponents) {
			...
			StatisticsCaseAttribute attribute = StatisticsHelper.getEnum(filterComponent.getSelectedAttribute());
			...
			case AGE_INTERVAL_BASIC:
				if (filterElement.getSelectedValues() != null) {
					...
					if (showCaseIncidence && filterComponent.getSelectedAttributeEnum() == StatisticsAttributeEnum.AGE_INTERVAL_5_YEARS) {
