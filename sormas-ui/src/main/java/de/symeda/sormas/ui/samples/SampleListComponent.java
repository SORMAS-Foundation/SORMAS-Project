...
public class SampleListComponent extends AbstractView {
	...
	public HorizontalLayout createTopBar() {
		...
		{
			Button statusAll = new Button("all", e -> grid.setShipmentStatusFilter(null));
			statusAll.setStyleName(ValoTheme.BUTTON_LINK);
			buttonFilterLayout.addComponent(statusAll);
			
			for(ShipmentStatus status : ShipmentStatus.values()) {
				Button statusButton = new Button(status.toString(), e -> grid.setShipmentStatusFilter(status));
