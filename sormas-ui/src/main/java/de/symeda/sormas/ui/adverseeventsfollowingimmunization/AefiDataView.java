...
import de.symeda.sormas.ui.adverseeventsfollowingimmunization.aefiinvestigationlink.AefiInvestigationListComponent;
...
public class AefiDataView extends AbstractAefiDataView {
	...
	protected void initView(String params) {
		...
		if (!isCreateAction
			&& (aefi.getSerious() == YesNoUnknown.YES)
			&& currentUser.hasAllUserRights(UserRight.ADVERSE_EVENTS_FOLLOWING_IMMUNIZATION_VIEW)) {

			AefiInvestigationListCriteria listCriteria = new AefiInvestigationListCriteria.Builder(getReference()).build();
			AefiInvestigationListComponent aefiInvestigationListComponent =
				new AefiInvestigationListComponent(listCriteria, this::showUnsavedChangesPopup, isEditAllowed(), isCreateAction);
			CssStyles.style(aefiInvestigationListComponent, CssStyles.VIEW_SECTION, CssStyles.VSPACE_TOP_3);

			layout.addSidePanelComponent(aefiInvestigationListComponent, INVESTIGATIONS_LOC);
		}
