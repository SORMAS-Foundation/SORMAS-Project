...
public class AefiInvestigationFilterForm extends AbstractFilterForm<AefiInvestigationCriteria> {
	...
	public AefiInvestigationFilterForm() {
		super(
			AefiInvestigationCriteria.class,
			AefiInvestigationCriteria.I18N_PREFIX,
			JurisdictionFieldConfig.of(AefiInvestigationCriteria.REGION, AefiInvestigationCriteria.DISTRICT, AefiInvestigationCriteria.COMMUNITY));
	}
	...
	protected String[] getMainFilterLocators() {
		return new String[] {
			AefiInvestigationCriteria.DISEASE,
			AefiInvestigationCriteria.PERSON_LIKE,
			AefiInvestigationCriteria.VACCINE_NAME,
			AefiInvestigationCriteria.REGION,
			AefiInvestigationCriteria.DISTRICT,
			AefiInvestigationCriteria.COMMUNITY,
			AefiInvestigationCriteria.STATUS_ON_DATE_OF_INVESTIGATION,
	...
	protected void addFields() {
		addField(FieldConfiguration.pixelSized(AefiInvestigationCriteria.DISEASE, 140));

		final TextField searchField = addField(
			FieldConfiguration
				.withCaptionAndPixelSized(AefiInvestigationCriteria.PERSON_LIKE, I18nProperties.getString(Strings.promptPersonsSearchField), 200));
		searchField.setNullRepresentation("");

		addFields(
			FieldConfiguration.pixelSized(AefiInvestigationCriteria.AEFI_TYPE, 140),
			FieldConfiguration.pixelSized(AefiInvestigationCriteria.VACCINE_NAME, 140));

		if (currentUserDto().getRegion() == null) {
			ComboBox regionFilter = addField(getContent(), FieldConfiguration.pixelSized(AefiInvestigationCriteria.REGION, 140));
			regionFilter.addItems(FacadeProvider.getRegionFacade().getAllActiveByServerCountry());
		}

		ComboBox districtFilter = addField(getContent(), FieldConfiguration.pixelSized(AefiInvestigationCriteria.DISTRICT, 140));
		...
		addField(getContent(), FieldConfiguration.pixelSized(AefiInvestigationCriteria.COMMUNITY, 140));

		addField(FieldConfiguration.pixelSized(AefiInvestigationCriteria.STATUS_ON_DATE_OF_INVESTIGATION, 140));
		addField(FieldConfiguration.pixelSized(AefiInvestigationCriteria.AEFI_CLASSIFICATION, 140));
	}
	...
	protected void applyDependenciesOnFieldChange(String propertyId, Property.ValueChangeEvent event) {
		...
		final AefiInvestigationCriteria criteria = getValue();

		final ComboBox facilityTypeGroupField = getField(AefiInvestigationCriteria.FACILITY_TYPE_GROUP);
		final ComboBox facilityTypeField = getField(AefiInvestigationCriteria.FACILITY_TYPE);
		final ComboBox facilityField = getField(AefiInvestigationCriteria.HEALTH_FACILITY);
		...
		switch (propertyId) {
		case AefiInvestigationCriteria.REGION: {
		...
		case AefiInvestigationCriteria.DISTRICT: {
		...
		case AefiInvestigationCriteria.COMMUNITY: {
		...
		case AefiInvestigationCriteria.FACILITY_TYPE_GROUP: {
			...
			if (!DataHelper.equal(typeGroup, criteria.getFacilityTypeGroup())) {
				if (typeGroup != null) {
					enableFields(AefiInvestigationCriteria.FACILITY_TYPE);
		...
		case AefiInvestigationCriteria.FACILITY_TYPE: {
	...
	@Override
	protected void applyDependenciesOnNewValue(AefiInvestigationCriteria criteria) {
		...
		EpiWeekAndDateFilterComponent<AefiInvestigationDateType> weekAndDateFilter =
			(EpiWeekAndDateFilterComponent<AefiInvestigationDateType>) dateFilterLayout.getComponent(0);

		AefiInvestigationDateType aefiInvestigationDateType = criteria.getAefiInvestigationDateType();
		weekAndDateFilter.getDateTypeSelector().setValue(aefiInvestigationDateType);
	...
	private HorizontalLayout buildWeekAndDateFilter() {
		...
		EpiWeekAndDateFilterComponent<AefiInvestigationDateType> weekAndDateFilter = new EpiWeekAndDateFilterComponent<>(
			false,
			false,
			null,
			AefiInvestigationDateType.values(),
			I18nProperties.getString(Strings.promptAefiInvestigationDateType),
			null,
			this);
		weekAndDateFilter.getWeekFromFilter().setInputPrompt(I18nProperties.getString(Strings.promptAefiInvestigationEpiWeekFrom));
		weekAndDateFilter.getWeekToFilter().setInputPrompt(I18nProperties.getString(Strings.promptAefiInvestigationEpiWeekTo));
		weekAndDateFilter.getDateFromFilter().setInputPrompt(I18nProperties.getString(Strings.promptAefiInvestigationDateFrom));
		weekAndDateFilter.getDateToFilter().setInputPrompt(I18nProperties.getString(Strings.promptAefiInvestigationDateTo));
	...
	private void onApplyClick(EpiWeekAndDateFilterComponent<AefiInvestigationDateType> weekAndDateFilter) {
		AefiInvestigationCriteria criteria = getValue();
		...
		if ((fromDate != null && toDate != null) || (fromDate == null && toDate == null)) {
			...
			AefiInvestigationDateType aefiInvestigationDateType = (AefiInvestigationDateType) weekAndDateFilter.getDateTypeSelector().getValue();
			criteria.setAefiInvestigationDateType(aefiInvestigationDateType);
