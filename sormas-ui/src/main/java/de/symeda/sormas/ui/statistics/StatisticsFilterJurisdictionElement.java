...
public class StatisticsFilterJurisdictionElement extends StatisticsFilterElement {
    ...
    public StatisticsFilterJurisdictionElement(int rowIndex) {
        ...
        regionElement = new StatisticsFilterValuesElement(I18nProperties.getPrefixCaption(CaseDataDto.I18N_PREFIX, CaseDataDto.REGION),
                StatisticsCaseAttribute.JURISDICTION, StatisticsCaseSubAttribute.REGION, this, rowIndex);
        districtElement = new StatisticsFilterValuesElement(I18nProperties.getPrefixCaption(CaseDataDto.I18N_PREFIX, CaseDataDto.DISTRICT),
                StatisticsCaseAttribute.JURISDICTION, StatisticsCaseSubAttribute.DISTRICT, this, rowIndex);
        communityElement = new StatisticsFilterValuesElement(I18nProperties.getPrefixCaption(CaseDataDto.I18N_PREFIX, CaseDataDto.COMMUNITY),
                StatisticsCaseAttribute.JURISDICTION, StatisticsCaseSubAttribute.COMMUNITY, this, rowIndex);
        healthFacilityElement = new StatisticsFilterValuesElement(I18nProperties.getPrefixCaption(CaseDataDto.I18N_PREFIX, CaseDataDto.HEALTH_FACILITY),
                StatisticsCaseAttribute.JURISDICTION, StatisticsCaseSubAttribute.HEALTH_FACILITY, this, rowIndex);
        ...
        regionElement.setValueChangeListener(e -> {
            ...
            if (regionElement.getSelectedValues() != null && districtElement.getSelectedValues() != null) {
                ...
                Map<String, String> districtRegionsMap = FacadeProvider.getDistrictFacade().getRegionUuidsForDistricts(districtValues.stream()
                        .map(d -> (DistrictReferenceDto) d.getValue())
                        .collect(Collectors.toList()));

                districtValues.stream().filter(d -> !regionUuids.contains(districtRegionsMap.get(((DistrictReferenceDto) d.getValue()).getUuid()))).forEach(d -> {
        ...
        districtElement.setValueChangeListener(e -> {
            ...
            if (districtElement.getSelectedValues() != null) {
                List<TokenizableValue> districtValues = districtElement.getSelectedValues();
                List<String> districtUuids = districtValues.stream().map(d -> ((DistrictReferenceDto) d.getValue()).getUuid()).collect(Collectors.toList());

                if (communityElement.getSelectedValues() != null) {
                    List<TokenizableValue> communityValues = communityElement.getSelectedValues();
                    Map<String, String> communityDistrictsMap = FacadeProvider.getCommunityFacade().getDistrictUuidsForCommunities(communityValues.stream()
                            .map(c -> (CommunityReferenceDto) c.getValue())
                            .collect(Collectors.toList()));
                    communityValues.stream().filter(c -> !districtUuids.contains(communityDistrictsMap.get(((CommunityReferenceDto) c.getValue()).getUuid()))).forEach(c -> {
                ...
                if (healthFacilityElement.getSelectedValues() != null) {
                    ...
                    Map<String, String> facilityDistrictsMap = FacadeProvider.getFacilityFacade().getDistrictUuidsForFacilities(facilityValues.stream()
                            .map(f -> (FacilityReferenceDto) f.getValue())
                            .collect(Collectors.toList()));
                    facilityValues.stream().filter(f -> !districtUuids.contains(facilityDistrictsMap.get(((FacilityReferenceDto) f.getValue()).getUuid()))).forEach(f -> {
        ...
        communityElement.setValueChangeListener(e -> {
            ...
            if (communityElement.getSelectedValues() != null && healthFacilityElement.getSelectedValues() != null) {
                List<TokenizableValue> communityValues = communityElement.getSelectedValues();
                List<String> communityUuids = communityValues.stream().map(c -> ((CommunityReferenceDto) c.getValue()).getUuid()).collect(Collectors.toList());
                List<TokenizableValue> facilityValues = healthFacilityElement.getSelectedValues();
                Map<String, String> facilityCommunitiesMap = FacadeProvider.getFacilityFacade().getCommunityUuidsForFacilities(facilityValues.stream()
                        .map(f -> (FacilityReferenceDto) f.getValue())
                        .collect(Collectors.toList()));
                facilityValues.stream().filter(f -> !communityUuids.contains(facilityCommunitiesMap.get(((FacilityReferenceDto) f.getValue()).getUuid()))).forEach(f -> {
                    healthFacilityElement.getTokenField().removeTokenizable(f);
                });
            }
        });
