...
public class ContactController {
    ...
    public void create(@NotNull final SormasUI ui) {
        create(ui, null, false, null);
    }

    public void create(@NotNull final SormasUI ui, CaseReferenceDto caseRef) {
        create(ui, caseRef, false, null);
    }

    /**
     * @param asResultingCase     Determines whether the case should be set as resulting case instead of source case
     * @param alternativeCallback Callback that is executed instead of opening the created contact
     */
    public void create(@NotNull final SormasUI ui, CaseReferenceDto caseRef, boolean asResultingCase, Runnable alternativeCallback) {
        ...
        CommitDiscardWrapperComponent<ContactCreateForm> createComponent =
                getContactCreateComponent(ui, caze, asResultingCase, alternativeCallback, false);
        VaadinUiUtil.showModalPopupWindow(createComponent, I18nProperties.getString(Strings.headingCreateNewContact));
    }

    public void create(@NotNull final SormasUI ui, EventParticipantReferenceDto eventParticipantRef) {
        ...
        CommitDiscardWrapperComponent<ContactCreateForm> createComponent = getContactCreateComponent(ui, eventParticipant);
        VaadinUiUtil.showModalPopupWindow(createComponent, I18nProperties.getString(Strings.headingCreateNewContact));
    }
    ...
    private ContactDto createNewContact(@NotNull final SormasUI ui, CaseDataDto caze, boolean asSourceContact) {
        ...
        setDefaults(ui.getUserProvider().getUser(), contact);

        return contact;
    }

    private ContactDto createNewContact(@NotNull final SormasUI ui, EventParticipantDto eventParticipant) {
    ...
    private void setDefaults(@NotNull UserDto user, ContactDto contact) {
        contact.setReportingUser(user.toReference());
        contact.setReportingDistrict(user.getDistrict());
    }

    private ContactDto createNewContact(@NotNull final SormasUI ui, EventParticipantDto eventParticipant, Disease disease) {
        ContactDto contact = createNewContact(ui, eventParticipant);
        contact.setDisease(disease);

        return contact;
    }

    public CommitDiscardWrapperComponent<ContactCreateForm> getContactCreateComponent(@NotNull final SormasUI ui, CaseDataDto caze) {
        return getContactCreateComponent(ui, caze, false, null, false);
    }

    /**
     * @param asSourceContact     Determines whether the case should be set as resulting case instead of source case
     * @param alternativeCallback Callback that is executed instead of opening the created contact
     */
    public CommitDiscardWrapperComponent<ContactCreateForm> getContactCreateComponent(
        ...
        createForm.setValue(createNewContact(ui, caze, asSourceContact));
        ...
        final CommitDiscardWrapperComponent<ContactCreateForm> createComponent = new CommitDiscardWrapperComponent<ContactCreateForm>(
                createForm,
                ui.getUserProvider().hasUserRight(UserRight.CONTACT_CREATE),
                ...
                if (asSourceContact && alternativeCallback != null && casePerson != null) {
                    selectOrCreateContact(dto, casePerson, I18nProperties.getString(Strings.infoSelectOrCreateContact), selectedContactUuid -> {
                        if (selectedContactUuid != null) {
                            ...
                            selectedContact.setResultingCaseUser(ui.getUserProvider().getUserReference());
    ...
    public CommitDiscardWrapperComponent<ContactCreateForm> getContactCreateComponent(
			@NotNull final SormasUI ui, EventParticipantDto eventParticipant) {
        ...
        createForm.setValue(createNewContact(ui, eventParticipant, disease));
    ...
    public CommitDiscardWrapperComponent<ContactDataForm> getContactDataEditComponent(
        ...
        final CommitDiscardWrapperComponent<ContactDataForm> editComponent = new CommitDiscardWrapperComponent<ContactDataForm>(
                editForm,
                ui.getUserProvider().hasUserRight(UserRight.CONTACT_EDIT),
    ...
    public CommitDiscardWrapperComponent<EpiDataForm> getEpiDataComponent(
			@NotNull final SormasUI ui, final String contactUuid) {
