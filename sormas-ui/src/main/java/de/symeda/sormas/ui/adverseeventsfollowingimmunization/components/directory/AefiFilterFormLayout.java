...
public class AefiFilterFormLayout extends VerticalLayout {

	private static final float PERCENTAGE_WIDTH = 100;

	private final AefiFilterForm filterForm;

	public AefiFilterFormLayout() {
		setSpacing(false);
		setMargin(false);
		setWidth(PERCENTAGE_WIDTH, Unit.PERCENTAGE);

		filterForm = new AefiFilterForm();
		addComponent(filterForm);
	}

	public AefiCriteria getValue() {
		return filterForm.getValue();
	}

	public void setValue(AefiCriteria criteria) {
